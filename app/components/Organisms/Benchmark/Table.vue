...
const rows = computed<Row[]>(() => {
  ...
  return Object.entries(scenario).map(([manager, result]) => {
    ...
    const isSuccess = Array.isArray(codes) && codes.length > 0 && codes.some((c: number) => c === 0)

    const isMixed = isSuccess && codes.some((c: number) => c !== 0)

    const finalExitCode = !isSuccess && codes.length > 0
        ? (codes.find((c: number) => c !== 0) ?? -1)
        : 0

    let meanTime = 0
    const times: number[] = result.time?.times || []
    if (isSuccess && Array.isArray(times) && times.length === codes.length) {
      const validTimes = times.filter((_val, idx) => codes[idx] === 0)
      if (validTimes.length > 0) {
        meanTime = validTimes.reduce((a, b) => a + b, 0) / validTimes.length
      }
    }

    let memory_mb = 0
    const memArray: number[] = result.time?.memory_usage_byte || []
    if (isSuccess && Array.isArray(memArray) && memArray.length === codes.length) {
      const firstValidMem = memArray.find((_val, idx) => codes[idx] === 0)
      if (firstValidMem) {
        memory_mb = Math.round(firstValidMem / 1024 / 1024)
      }
    }
...
const fastestTime = computed(() => {
  const validRows = rows.value.filter(r => r.isSuccess && r.mean > 0)
  return validRows.length > 0 ? Math.min(...validRows.map((r) => r.mean)) : 0
})
      ...
      <UTable :data="rows" :columns="columns">
        ...
        <template #mean-cell="{ row }">
          <div v-if="row.original.isSuccess" class="flex items-center gap-2">
            ...
            <UBadge
                v-if="row.original.mean === fastestTime && row.original.mean > 0"
                label="Najszybszy"
                color="success"
                variant="subtle"
                size="xs"
